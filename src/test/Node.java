...
public class Node {
    ...
    public static Node plantTree(Integer[] nums) {
        if (nums.length == 0)
            return new Node(0);
        Deque<Node> nodeQueue = new LinkedList<>(); // 创建一个根节点
    ...
    public static void showNext(Node root) {
        if (root == null) return;
        Queue<Node> queue = new LinkedList<>();
        queue.offer(root);
        while (!queue.isEmpty()) {
            root = queue.poll();
            System.out.println("node:[" + root + ":" + root.val + "] "
                    + "node.next:[" + root.next + ":" + (root.next == null ? null : root.next.val) + "]");
